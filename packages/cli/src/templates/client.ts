...
export function generateClientCode(_ir: IR): string {
  ...
  L.push(
    "  const url = new URL(opts.path.startsWith('/') ? opts.path.slice(1) : opts.path, base);",
  );
  L.push(
  ...
  L.push(
    "    body: opts.body ? JSON.stringify(opts.body) : undefined, signal: AbortSignal.timeout(TIMEOUT) };",
  );
  L.push("  for (let i = 0; i <= MAX_RETRY; i++) {");
  L.push("    const res = await fetch(url.toString(), init);");
  L.push(
    "    if (res.ok) { const txt = await res.text(); try { return JSON.parse(txt); } catch { return txt; } }",
  );
  L.push(
    "    if (!RETRY_ON.has(res.status) || i === MAX_RETRY) throw new AppError(res.status, await res.text());",
  );
  ...
  L.push(
    "  const res = await fetch(BASE_URL, { method: 'POST', headers, body, signal: AbortSignal.timeout(TIMEOUT) });",
  );
  ...
  L.push(
    '  const verbs: Record<string, string> = { GET: "list", POST: "create", DELETE: "delete", PATCH: "update" };',
  );
  L.push('  const hasId = path.includes("{");');
  L.push(
    '  const verb = method === "GET" && hasId ? "get" : (verbs[method] ?? method.toLowerCase());',
  );
