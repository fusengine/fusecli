...
function emitTableLines(): string[] {
  const bt = "`";
  const n1 = `        if (Array.isArray(nv)) { for (const item of nv) console.log(pc.dim(${bt}  > ${`${bt}$`}{flat(item)}${bt})); }`;
  const n2 = `        else if (nv != null) console.log(pc.dim(${bt}  > ${`${bt}$`}{pc.bold(nk)}: ${`${bt}$`}{flat(nv)}${bt}));`;
  return [
