...
function emitTableLines(): string[] {
  ...
  return [
    ...
    "  let scalarKeys = allKeys.filter(k => typeof rows[0]![k] !== 'object' || rows[0]![k] == null);",
    "  if (scalarKeys.includes('url') && scalarKeys.includes('id')) scalarKeys = scalarKeys.filter(k => k !== 'id');",
    "  const nestedKeys = allKeys.filter(k => typeof rows[0]![k] === 'object' && rows[0]![k] != null);",
    "  const maxW = Math.max(10, Math.floor((cols - scalarKeys.length * 2) / scalarKeys.length));",
    "  const w = scalarKeys.map(k => Math.min(maxW, Math.max(k.length, ...rows.map(r => flat(r[k]).length))));",
    "  if (!noH) console.log(scalarKeys.map((k, i) => pc.bold(k.padEnd(w[i]!))).join('  '));",
    "  for (const r of rows) {",
    "    console.log(scalarKeys.map((k, i) => flat(r[k]).slice(0, w[i]!).padEnd(w[i]!)).join('  '));",
